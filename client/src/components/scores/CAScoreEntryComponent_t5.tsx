...
interface CAScoreEntryComponentProps {
  ...
  onSaveStudent?: (
    studentId: string,
    scoreData: StudentDetailedScore
  ) => Promise<boolean>; // NEW: For individual student saves
}
...
const CAScoreEntryComponent: React.FC<CAScoreEntryComponentProps> = ({
  ...
  initialScores,
  onReconfigure,
  onSaveStudent, // NEW: For individual student saves
}) => {
  // State for all scores (by student ID)
  const [scores, setScores] = useState<DetailedScore>({});

  // NEW: State for tracking which students are being saved
  const [savingStudents, setSavingStudents] = useState<{
    [studentId: string]: boolean;
  }>({});

  // NEW: State for tracking which students were successfully saved
  ...
  const handleScoreChange = useCallback(
    (
      ...
      // Reset saved status for this student when score changes
  ...
  const handleSaveStudent = async (studentId: string) => {
    ...
    try {
      // Mark student as saving
      setSavingStudents((prev) => ({ ...prev, [studentId]: true }));

      // Get the student's scores
      const studentScore = scores[studentId];
      if (!studentScore) {
        console.error(`No scores found for student ${studentId}`);
        return;
      }

      // Ensure the test date is set
      if (!studentScore.testDate) {
        studentScore.testDate = formatDateForStorage(testDate);
      }

      // Call the parent save function
      const success = await onSaveStudent(studentId, studentScore);

      // Update save status
      setSavedStudents((prev) => ({ ...prev, [studentId]: success }));

      console.log(
        `Student ${studentId} save ${success ? "succeeded" : "failed"}`
      );
        ...
        <Table size="small">
          ...
          <TableBody>
            {students.map((student, index) => {
              ...
              // Check if this student is currently being saved
              const isSaving = savingStudents[student._id] || false;

              // Check if this student has been saved successfully
              const isSaved = savedStudents[student._id] || false;
              ...
              return (
                ...
                >
                  ...
                  {onSaveStudent && (
                    <TableCell align="center">
                      <Button
                        variant={isSaved ? "text" : "contained"}
                        color={isSaved ? "success" : "primary"}
                        size="small"
                        onClick={() => handleSaveStudent(student._id)}
                        disabled={isSaving}
                        startIcon={isSaved ? <CheckIcon /> : null}
                        sx={{ minWidth: "90px" }}
                      >
                        {isSaving ? (
